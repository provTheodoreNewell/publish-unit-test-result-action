...
class TestNunit(unittest.TestCase):
    def test_process_parse_nunit_files_with_time_factor(self):
        result_file = str(test_files_path / 'nunit.xml')
        for time_factor in [1.0, 10.0, 60.0, 0.1, 0.001]:
            with self.subTest(time_factor=time_factor):
                actual = process_junit_xml_elems(parse_nunit_files([result_file]), time_factor=time_factor)
                print(actual)
